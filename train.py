...
logging.basicConfig(filename=save_path + 'TranSNP_Net.log',
                    format='[%(asctime)s-%(filename)s-%(levelname)s:%(message)s]', level=logging.INFO, filemode='a',
                    datefmt='%Y-%m-%d %I:%M:%S %p')
logging.info("TranSNP_Net-Train")
model = TranSNP_Net()

num_parms = 0
if (opt.load_pre is not None and opt.stare_epoch == 1):
    model.load_pre(opt.load_pre)
    print('load model from ', opt.load_pre)
else:
    model.load_state_dict(torch.load('./cpts/TranSNP_Net_epoch_60.pth'), strict=False)
    print('load model from ', './cpts/TranSNP_Net_epoch_60.pth')
...
def train(train_loader, model, optimizer, epoch, save_path):
    ...
    try:
        ...
        if (epoch) % 5 == 0:
            torch.save(model.state_dict(), save_path + 'TranSNP_Net_epoch_{}.pth'.format(epoch))
    except KeyboardInterrupt:
        print('Keyboard Interrupt: save model and exit.')
        if not os.path.exists(save_path):
            os.makedirs(save_path)
        torch.save(model.state_dict(), save_path + 'TranSNP_Net_epoch_{}.pth'.format(epoch + 1))
    ...
    with torch.no_grad():
        ...
        if epoch == 1:
            ...
            if mae < best_mae:
                best_mae = mae
                best_epoch = epoch
                torch.save(model.state_dict(), save_path + 'TranSNP_Net_epoch_best.pth')
...
if __name__ == '__main__':
    print("Start train...")
    for epoch in range(opt.stare_epoch, opt.epoch):
