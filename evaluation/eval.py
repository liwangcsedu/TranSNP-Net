...
def main(cfg,model_path):
    ...
    #写入当前评估的模型路径到result.txt
    with open('./score/result.txt', 'a') as f:
        f.write(model_path + '\n')
...
#     pred_root_dir= './results/pred_maps/'
#     gt_root_dir='./results/gt/'
...
#     parser.add_argument('--save_dir', type=str, default='./results/score/')
#     parser.add_argument('--cuda', type=bool, default=True)
#     config = parser.parse_args()
#     main(config)

def eval(model_path):
    pred_root_dir= './test_maps/'
    gt_root_dir= './data/truth/'
    MODEL_NAMES = os.listdir(pred_root_dir)
    MODEL_NAMES.sort(key=lambda x: x[:])
    DATA_NAMES = os.listdir(gt_root_dir)
    DATA_NAMES.sort(key=lambda x: x[:])
